...
Client.prototype.request = function(method, filename, headers){
  ...
  // user proxy if HTTP_PROXY or http_proxy enf are set
  var envProxy = process.env.HTTP_PROXY || process.env.http_proxy;
  var useProxy = !!envProxy; // is ture when string and false when empty string ""
  if(useProxy) {
    ...
    options.host = pEnvProxy.hostname
    options.port = pEnvProxy.port  
  }
